...
test-e2e:
	@if [ ! -f web/nginx.conf ]; then echo "Error: web/nginx.conf no existe"; exit 1; fi
	...
	docker exec -it calc-web ls -l /etc/nginx/conf.d/
	docker run --rm --volume `pwd`/test/e2e/cypress.json:/cypress.json --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results --network calc-test-e2e cypress/included:4.9.0 --browser chrome --headless || true
	docker rm -v --force apiserver
	docker rm -v --force calc-web
